...
def add_date_to_pdf(input_pdf_path, output_pdf_path, year, month, day):
    ...
    try:
        ...
        # 座標を計算（左上から：上端から22.5mm下、左端から156mm右）
        margin_from_top = 22.5 * MM_TO_POINTS
        margin_from_left = 156 * MM_TO_POINTS

        # PDF座標系での位置
        # PyMuPDFの座標系：左上が原点(0,0)、y軸は上から下に増加
        # したがって、上端から22.5mm下の位置は単純に margin_from_top
        x = margin_from_left
        y = margin_from_top
        ...
        if font_path:
            ...
            # 固定の会社名テキストを追加（左端から115mm、上端から107mm）
            company_text = "インフォコネクト株式会社"
            company_x = 115 * MM_TO_POINTS
            company_y = 107 * MM_TO_POINTS
